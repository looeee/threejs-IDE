...
class Panels {
  ...
  restoreState() {
    ...
    Object.keys(this.panels).forEach((name) => {
      const panel = this.panels[name];
      if (state[name]) {
        if (state[name].sizes) panel.split.setSizes(state[name].sizes);
  ...
  createSplit(panel) {
    return new Split(panel.config.selectors, {
      ...
      elementStyle:
        panel.config.elementStyle || this.config.elementStyle || undefined,
      gutterSize: panel.config.gutterSize || this.config.gutterSize || 1,
      gutterStyle:
        panel.config.gutterStyle || this.config.gutterStyle || undefined,
